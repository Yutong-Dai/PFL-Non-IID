...
def read_data(dataset, idx, yd, is_train=True):
    if is_train:
        train_data_dir = os.path.join('../dataset', dataset, 'train/')
        if yd:
            train_data_dir = os.path.join('../dataset', dataset, 'yd_train/')
        ...
        if yd:
            test_data_dir = os.path.join('../dataset', dataset, 'yd_test/')
...
def read_client_data(dataset, idx, yd, is_train=True):
    if dataset[:2] == "ag" or dataset[:2] == "SS":
        return read_client_data_text(dataset, idx)

    if is_train:
        if yd:
            train_data = read_data(dataset, idx, yd, is_train)
        else:
            train_data = read_data(dataset, idx, yd, is_train)
        ...
        if yd:
            test_data = read_data(dataset, idx, yd, is_train)
        else:
            test_data = read_data(dataset, idx, yd, is_train)
